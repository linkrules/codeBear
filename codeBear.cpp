...
/*
void InitCommentStr( int commentID ) {
	switch( commentID ) {
	case 0:
...
void InsertCommentControl (){
	//if( isControlPressed && isAltPressed && isSpacePressed && !isInsertLock ){
	if( KEYDOWN( VK_CONTROL ) && KEYDOWN( VK_MENU ) && KEYDOWN( VK_SPACE ) && !isInsertLock ){
		isInsertLock = true;
	...
	if( KEYUP( VK_CONTROL ) && KEYUP( VK_MENU ) && KEYUP( VK_SPACE ) && isInsertLock ){
		//sprintf(buffer,"");
		isInsertLock = false;
	}
